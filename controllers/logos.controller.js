...
exports.getAllLogos = async (req, res, next) => {
  try {
    ...
    const logos = await db.Logos.findAll({
...
exports.createLogo = async (req, res, next) => {
  try {
    const newLogo = await db.Logos.create(req.body);
...
exports.getLogoById = async (req, res, next) => {
  try {
    const { id } = req.params;
    const logo = await db.Logos.findByPk(id);
...
exports.updateLogo = async (req, res, next) => {
  try {
    const { id } = req.params;
    const [updatedRowsCount] = await db.Logos.update(req.body, {
      where: { logo_id: id },
    });
    ...
    const updatedLogo = await db.Logos.findByPk(id);
...
exports.deleteLogo = async (req, res, next) => {
  try {
    const { id } = req.params;
    const deletedRowCount = await db.Logos.destroy({
      where: { logo_id: id },
    });
