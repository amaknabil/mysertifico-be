...
const addBoUserHandler = asyncHandler(async (req, res) => {
  ...
  if (!full_name || !email || !role_name) {
    throw new CustomError("Please provide full name, email, and role name.", 400);
  }
  ...
  try {
    ...
    if (!boApp) {
        throw new CustomError("Back Office application not found in database.", 500);
    }
    ...
    if (!role) {
        throw new CustomError(`Role '${role_name}' not found for the Back Office application.`, 404);
    }
    ...
    if (!existingUser) {
      ...
      if (existingRole) {
          throw new CustomError(`User ${email} already has the role '${role_name}'.`, 409);
      }
    }
  ...
  try {
    const emailClient = new Email({ email: user.email, full_name: user.full_name });
    if (isNewUser && tempPassword) {
      ...
      console.error(`Email failed to send to ${user.email} after BO user creation:`, emailError);
  }
...
module.exports = { updateBOHandler, addBoUserHandler };
